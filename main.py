...
class MyAPP(QMainWindow):
    def __init__(self):
        ...
        self.window = CorrectionWindow()  # window for
        ...
        # correction table
        self.ui.set_correction_toolButton.clicked.connect(self.show_correction_window)
        self.window.add_tool_button.clicked.connect(self.window.add_row)
        self.window.delete_tool_button.clicked.connect(self.window.delete_row)
        self.window.upload_tool_button.clicked.connect(self.window.upload_table)
        self.window.set_correction_tool_button.clicked.connect(
            lambda: self.window.set_correction(self.ui.travel_times_listWidget))
        self.window.save_tool_button.clicked.connect(self.window.save_table)
    ...
    def del_button_enable_status_changed(self, list_widget, del_button):
        if list_widget.count() and len(list_widget.selectedItems()):
            ...
            if del_button == self.ui.travel_times_del:
                self.ui.set_correction_toolButton.setEnabled(True)
        else:
            del_button.setDisabled(True)
            self.ui.plot_pushButton.setDisabled(True)
            if del_button == self.ui.travel_times_del:
                self.ui.set_correction_toolButton.setDisabled(True)
    ...
    def delete_row(self):
        index_list = []
        for model_index in self.ui.tensor_moments_tableWidget.selectionModel().selectedRows():
            index = QtCore.QPersistentModelIndex(model_index)
            index_list.append(index)
        for index in index_list:
            self.ui.tensor_moments_tableWidget.removeRow(index.row())
